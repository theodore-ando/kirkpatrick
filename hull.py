...
from polygons import Segment, Point
...
def split_tri(A: Point, u: Point, B: Point, points: List[Point]) -> Tuple[List[Point], List[Point]]:
    """
...
def quickhull(points: List[Point]) -> List[Point]:
    ...
    B = max(points, key=lambda p: p.x)

    points.remove(A)
    points.remove(B)

    points_up, points_down = split(A, B, points)

    points.append(A)
    points.append(B)

    return [A] + upperhull(B, A, points_down) + \
           [B] + upperhull(A, B, points_up)